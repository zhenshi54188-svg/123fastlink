...
export async function create189RapidTransfer(shareUrl, sharePwd, writer) {
  ...
  const signature = get189Signature({ ...params, Timestamp: timestamp, AppKey: appKey });
...
async function get189ShareFiles(shareId, shareDirFileId, fileId, path = "", shareMode = "0", accessCode = "", shareCode = "", writer, allFiles) {
  ...
  while (true) {
    ...
    for (const file of fileList) {
      allFiles.push({ path: path ? `${path}/${file.name}` : file.name, etag: (file.md5 || "").toLowerCase(), size: file.size });
...
function parseXMLResponse(xmlText) {
  const getTagValue = (xml, tagName) => xml.match(new RegExp(`<${tagName}>([^<]*)<\/${tagName}>`, 'i'))?.[1] || null;
  return {
    res_code: parseInt(getTagValue(xmlText, 'res_code') || '0'), res_message: getTagValue(xmlText, 'res_message') || '',
    shareId: getTagValue(xmlText, 'shareId') || '', fileId: getTagValue(xmlText, 'fileId') || '',
    shareMode: getTagValue(xmlText, 'shareMode') || '0', isFolder: getTagValue(xmlText, 'isFolder') === 'true',
    needAccessCode: getTagValue(xmlText, 'needAccessCode') || '0', fileName: getTagValue(xmlText, 'fileName') || ''
  };
}
...
function simpleMD5(str) {
  // MD5 implementation remains the same
  ...
  function convertToWordArray(str) {
    let lWordCount; let lMessageLength = str.length; let lNumberOfWords_temp1 = lMessageLength + 8;
    let lNumberOfWords_temp2 = (lNumberOfWords_temp1 - (lNumberOfWords_temp1 % 64)) / 64;
    let lNumberOfWords = (lNumberOfWords_temp2 + 1) * 16; let lWordArray = Array(lNumberOfWords - 1);
  ...
  let x = Array(); let k, AA, BB, CC, DD, a, b, c, d;
  ...
  let str_utf8 = Utf8Encode(str);
  x = convertToWordArray(str_utf8); a = 0x67452301; b = 0xEFCDAB89; c = 0x98BADCFE; d = 0x10325476;
